...
export default async function AdminMediaPage() {
  ...
  return (
    <AdminShell profile={profile} currentPath="/admin/media">
      <AdminPageHeader
        title="Média kezelő"
        description="Képfeltöltés és gyors kiválasztás bloghoz, oldalakhoz és hero blokkokhoz."
      />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {assets.map((asset) => (
            <article key={asset.name} className="card-surface rounded-[1.75rem] p-6">
              ...
              <a href={asset.url} target="_blank" className="mt-3 block text-sm text-moss">
                {asset.url}
              </a>
            </article>
          ))}
